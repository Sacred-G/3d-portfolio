...
const Model1 = ({ funcs, ghosted }: { funcs: any; ghosted: boolean }) => {
	const { nodes, animations } = useGLTF(modelGltf) as any
	...
	if (!nodes || Object.keys(nodes).length === 0) {
		console.error('Model1: No nodes loaded from build.glb')
		return null
	}
	...
	// Material.001 will be used directly from materials object

	useEffect(() => {
		if (actions['anthenaAction']) {
	...
	return (
		<group ref={group} dispose={null}>
			<group name="Scene" onClick={(ev) => ev.stopPropagation()}>
				...
				{/* Removed baked neon sign name_sign_frame */}
				{/* {safeNode('name_sign_frame') && (
					<mesh
